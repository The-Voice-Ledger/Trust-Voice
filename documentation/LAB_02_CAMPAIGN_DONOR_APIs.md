...
## 🎯 Lab Overview: Building the API Layer
...
- ✅ 3 complete API routers (NGOs, Campaigns, Donors)
- ✅ 15+ endpoints with proper HTTP status codes
- ✅ Input validation using Pydantic schemas
- ✅ Comprehensive error handling
- ✅ Seed data for testing
- ✅ 22 integration tests covering all endpoints
...
**Resource-Based URLs:**
```
/ngos/              → Collection of NGOs
/ngos/{id}          → Specific NGO
/campaigns/         → Collection of campaigns
/campaigns/{id}     → Specific campaign
/donors/            → Collection of donors
/donors/{id}        → Specific donor
```
...
voice/routers/
├── ngos.py        # NGO management
├── campaigns.py   # Campaign CRUD
└── donors.py      # Donor registration
```
...
- **Field Validation:**
  - `min_length`/`max_length` → Prevents empty strings and database overflow
  - `EmailStr` → Pydantic validates email format automatically
  - `pattern` → Regex validation for phone (E.164) and blockchain address
  - `...` → Required field (Pydantic syntax)
...
## 💻 Part 2: Campaign Management API
...
**🔍 Why Multiple Contact Validations:**
...
## 💻 Part 4: Database Updates & Router Registration
...
def seed_ngos():
    ...
    if existing_count > 0:
        print(f"✓ NGOs already seeded ({existing_count} found)")
        db.close()
        return
...
def seed_campaigns():
    ...
    if existing_count > 0:
        print(f"✓ Campaigns already seeded ({existing_count} found)")
        db.close()
        return
    ...
    if not all([water_ngo, edu_ngo, health_ngo]):
        print("✗ NGOs not found - run seed_ngos() first")
        db.close()
        return
...
def seed_donors():
    ...
    if existing_count > 0:
        print(f"✓ Donors already seeded ({existing_count} found)")
        db.close()
        return
...
```
🌱 Seeding database...
✓ NGOs already seeded (3 found)
✓ Campaigns already seeded (5 found)
✓ Donors already seeded (5 found)
✅ Database seeding complete!
```

---

## 🧪 Part 6: Testing the APIs
...
## 🧪 Part 7: Automated Integration Tests
...
2. **Lab 2:** APIs (CRUD operations) ← You are here
