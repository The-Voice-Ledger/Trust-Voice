...
export default function Dashboard() {
  ...
  return (
    <PageBg pattern="isometric" colorA="#14B8A6" colorB="#10B981">
    <div className="max-w-3xl mx-auto px-4 py-6">
      <PageHeader icon={null} title={t('dashboard.title')} accentColor="green" />

      {/* Stats cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <BespokeStatCard accent="#10B981" label={t('dashboard.total_donated')}>
          {Object.entries(totalsByFx).map(([cur, amt]) => (
            <span key={cur} className="block text-xl sm:text-2xl font-bold" style={{ color: '#10B981' }}>
        ...
        {taxSummary && (
          <>
            <BespokeStatCard accent="#14B8A6" label={`${t('dashboard.tax_year')} ${currentYear}`}>
              <span className="text-xl sm:text-2xl font-bold text-emerald-600">
                ${Number(taxSummary.total_deductible || taxSummary.total_amount || 0).toLocaleString()}
              </span>
            </BespokeStatCard>
            <BespokeStatCard accent="#0D9488" label={t('dashboard.receipts_count')}>
      ...
      {activeReceipt && (
        <div className="relative rounded-2xl bg-white/80 backdrop-blur-sm border border-gray-100 shadow-md p-6 mb-6 overflow-hidden">
          ...
          <svg className="absolute top-2 right-12 w-20 h-20 pointer-events-none" viewBox="0 0 80 80" fill="none">
            ...
            <circle cx="57" cy="42" r="4" stroke="#14B8A6" strokeWidth="0.3" opacity="0.04" />
            <path d="M55 42 L59 42" stroke="#14B8A6" strokeWidth="0.3" opacity="0.04" />
            <path d="M57 40 L57 44" stroke="#14B8A6" strokeWidth="0.3" opacity="0.04" />
          </svg>
      ...
      {loading ? (
        ...
        <div className="text-center py-10">
          ...
          <Link to="/campaigns" className="text-emerald-600 hover:underline text-sm font-medium">
