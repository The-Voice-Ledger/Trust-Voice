  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <Stat accent="#6366F1" icon={HiOutlineDocumentText} label="Campaigns" value={campaigns.length} />
        <Stat accent="#059669" icon={HiOutlineBanknotes} label="Total Raised" value={`$${fmt(totalRaised)}`} sub={`of $${fmt(totalGoal)}`} />
        <Stat accent="#A855F7" icon={HiOutlineCheckCircle} label="Milestones" value={`${completedMs}/${allMs.length}`} sub="completed" />
        <Stat accent="#7C3AED" icon={HiOutlineShieldCheck} label="Released" value={`$${fmt(totalReleased)}`} sub="to projects" />
      </div>
